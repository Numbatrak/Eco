// Generated via `npx @better-auth/cli generate --config apps/api/src/lib/auth.ts`
// against the organization + twoFactor plugin config. Hand-folded into this
...
export const organization = pgTable(
  "organization",
  {
    id: text("id").primaryKey(),
    name: text("name").notNull(),
    // Reused directly as the storefront subdomain - no separate field.
...
export const member = pgTable(
  "member",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    // "viewer" (least-privileged real role) rather than the generated
    // default of "member" - that role doesn't exist in this platform's
    // access-control matrix (owner/admin/editor/viewer). In practice every
    // insert path (signup, invite-accept) sets role explicitly.
