...
/**
 * Boxed one-time-code entry used for both TOTP and email OTP. Supports
 * pasting a full code (from a password manager, SMS autofill suggestion, or
 * copy-paste from an authenticator app) into any box, distributing it across
 * all boxes at once rather than requiring one digit typed per box.
 */
export function OtpInput({
  ...
  function handleChange(index: number, raw: string): void {
    ...
    if (!cleaned) {
      // Deletion via the native change event (rare - backspace is handled in onKeyDown).
      commit(value.slice(0, index) + value.slice(index + 1));
      return;
    }
    if (cleaned.length > 1) {
      // Some mobile keyboards/autofill deliver the whole code as one "change".
