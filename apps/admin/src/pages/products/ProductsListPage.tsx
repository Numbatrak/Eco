...
export default function ProductsListPage(): React.ReactElement {
  ...
  async function load(): Promise<void> {
    ...
    try {
      const { products: list } = await commerceApi.listProducts(accessToken, currentTenant.id);
      setProducts(list);
    } catch {
  ...
  async function handleDelete(): Promise<void> {
    if (!accessToken || !currentTenant || !deleteTarget) {
      return;
    }
    await commerceApi.deleteProduct(accessToken, currentTenant.id, deleteTarget.id);
  ...
  const filtered = (products ?? []).filter((product) => filter === "all" || product.status === filter);
                ...
                <tr key={product.id}>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                      ...
                      <button type="button" className="btn-link" onClick={() => setDeleteTarget(product)}>
                        Delete
                      </button>
                    </div>
                  </td>
